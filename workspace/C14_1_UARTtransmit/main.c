...
int main(void)
{
	...
	while(1)
	{
	    // check output pin from TXD; note: falling edge
	    ...
	    UCA1TXBUF = 'A';                // byte ASCII char format
	    for(i=0; i < 10000; i++){}    // delay
	    for(j=0; j < sizeof(message); j++)
	    {
	        UCA1TXBUF = message[j];       // byte ASCII char format
...
__interrupt void ISR_Port4_S1(void)
{
    ...
    UCA1TXBUF = message[msgIdx];        // send first byte
